...
!  Written by D. Vaughan Griffiths, Nov 16, 1999
!  Latest Update: Jul 23, 2002
!
!  PURPOSE  set up the element and node numbering for the slope problem
!           of RSLOPE2D.
!
!  DESCRIPTION
!
!  This routine sets up the node/element numbers and coordinates for
!  the slope problem. It is assumed that the slope looks like
!                                        y
!     --  +----------------              ^
!     ^   |                 \            |
!     |   |                  \           |
...
!         |<---- nx1 ---->|      |<--- nx2 ------->|
!
!  The number of elements in the x-direction through the embankment is
!  determined to be ny1*ngrad, so that the total number of elements
!  in the x-direction is (nx1 + ny1*ngrad + nx2) and the total number
!  of elements in the y-direction is (ny1 + ny2).
!
!  ARGUMENTS
!  The arguments nf, g_num, g_coord, nels, nn, and n are output.
!  All others are input.
!
!  REVISION HISTORY:
!  1.01	properly identified output variables above. (Jul 23/02)
!-----------------------------------------------------------------------
...
!
      ...
      do 8711 i=1,2*(ny4+1+(j-1)/ngrad1)
      nm=nm+1
      ic=ic+1
      nf(nm,1)=ic
      ic=ic+1
...
!
      ...
      coord(7,1)=coord(5,1)
      if(iq.eq.1.and.ip.eq.1)coord(3,1)=coord(3,1)+0.5*dx
...
!
      ...
      coord(5,1)=xshift+ip*dx
      coord(7,1)=coord(5,1)
      if(iq.eq.1.and.ip.eq.ngrad)coord(5,1)=coord(5,1)-0.5*dx
      coord(2,1)=.5*(coord(1,1)+coord(3,1))
      coord(4,1)=.5*(coord(3,1)+coord(5,1))
      coord(6,1)=.5*(coord(5,1)+coord(7,1))
      coord(8,1)=.5*(coord(7,1)+coord(1,1))
