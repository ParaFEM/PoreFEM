...
If(pmax.gt.pmaxb)then
pmaxb=pmax
else
pmax=pmaxb
endif
If(pmin.lt.pminb)then
pminb=pmin
else
!pmin=pminb
endif
...
!						write out title
      write(ips,1)'14 /Times-Roman findfont exch scalefont setfont'
      write(ips,5) 10., scale*ysize + 19., ' m'
      if( lfail ) then
...
! WRITE(ips, '(A,2F8.3,A)')'gsave c  ',gr,1.0-gr,' 0.0 setrgbcolor fill grestore'
   ...
   60 continue
!       barsize=abs(g_coord(1,g_num(1,1))-g_coord(1,g_num(7,1)))
       barsize=1.0
	   height=height*0.5
      do 61 i=1,101
            gr = (i-1)*0.01
	        WRITE(ips,'(f9.3,a)')gr,'   setgray'
            x=sx*(xmax+barsize- xmin)
            y  = sy*(height-(i-1)*0.1*barsize)
            write(ips,5) x, y,' m'
            x  = sx*(xmax+barsize- xmin)
            y  = sy*(height-i*0.1*barsize)
            write(ips,5) x, y,' l'
            x  = sx*(xmax+2*barsize- xmin)
            y  = sy*(height-i*0.1*barsize)
            write(ips,5) x, y,' l'
            x  = sx*(xmax+2*barsize- xmin)
            y  = sy*(height-(i-1)*0.1*barsize)
     ...
     61 continue
            WRITE(ips,'(a)')'0.0   setgray'
            x=sx*(xmax+barsize- xmin)
            y  = sy*height
            write(ips,5) x, y,' m'
            x  = sx*(xmax+barsize- xmin)
            y  = sy*(height-10*barsize)
            write(ips,5) x, y,' l'
            x  = sx*(xmax+2*barsize- xmin)
            y  = sy*(height-10*barsize)
            write(ips,5) x, y,' l'
      ...
      write(ips,1)'8  /Times-Roman findfont exch scalefont setfont'
      do 62 i=1,11
       write(ips,5) x, sy*(height-(i-1)*barsize)+1.5, ' m'
	   write(str,'(f12.2)') pmax-(i-1)*0.1*(pmax-pmin)
       str=adjustl(str)
       write(ips,1)'(_'//str//') show'
      62 continue
