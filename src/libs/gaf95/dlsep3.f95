...
!  Written by Gordon A. Fenton, Mar. 24, 1994
!  Latest Update: May 9, 2001
!
!  PURPOSE  returns the covariance between two points in a 3-D random field
!           having a separable Markovian covariance function.
!
!  This function returns the covariance between two points in the field
!  separated by lag vector {X,Y,Z}. The covariance function for this field
!  is given by
!
!      B(X,Y,Z) = var*exp{-(2|X|/dthx)}*exp{-(2|Y|/dthy)}*exp{-(2|Z|/dthz)}
!
!  where var is the point variance, and dthx, dthy, and dthz are the scales of
!  fluctuation in the x-, y-, and z-directions, respectively. These parameters
!  are brought in through the common block /dparam/.
!
!  NOTE: this function is NOT isotropic even if dthx = dthy = dthz.
!
...
!  separable and can be written as the product of three 1-D variance
!  functions corresponding to the Markovian correlation function
!
!         V(X,Y,Z) = V(X)*V(Y)*V(Z)
!
...
!                     4*X^2
!
!  The variance of the local average is then given by var*V(X,Y,Z). See page
