...
!  *                                                                 *
!  *******************************************************************
!  Double Precision Version 1.53
!  Written by Gordon A. Fenton, Sept. 1989
!  Latest Update: Jul 2, 2003
!
!  PURPOSE  returns the covariance between two points in a 3-D volume for
!           a random field having a Markovian covariance function.
!
!  This function returns the covariance between two
!  points in the field separated by lag vector {X,Y,Z}.
!  The 3-D Markovian process has covariance function
!
!   B(X,Y,Z) = var* exp{ -sqrt[(2|X|/dthx)^2 + (2|Y|/dthy)^2 + (2|Z|/dthz)^2] }
!
!  where var is the point variance of the process and dthx, dthy, and
!  dthz are the directional scales of fluctuation. The parameters var,
!  dthx, dthy, and dthz are brought in via the common block /dparam/.
!
!  Note that if var < 0, as provided in the common block /dparam/,
!  then this function returns the variance of a local average of the
!  process over a volume of side dimensions {X,Y,Z}. The `local average'
!  variance is obtained by Gaussian quadrature integration of the above
!  covariance function (the 6-D integral is collapsed to a 3-D integral by
!  taking advantage of the fact that the above function is quadrant
!  symmetric.
!  See "Random Fields" by E. Vanmarcke and G.A. Fenton's Ph.D. thesis for
!  more details on local average theory.
!
!  The arguments to this routine are just the elements of the lag vector
!  between points for which the covariance is desired.
!
!  REVISION HISTORY:
...
!  1.3	covariance version now returns isotropic Markovian model (Jun 22/00)
!  1.4	eliminated lvarfn -- now return covariances only if var < 0 (Apr 9/01)
!  1.5	reversed default - now return covariances if var > 0.
!	Variance function values now computed by Gauss quadrature (Apr 11/01)
!  1.51	revised the above description to reflect Revision 1.5 (May 9/01)
!  1.52	now use 16 pt Gauss quadrature for variance calcs (Apr 8/03)
!  1.53	now use 20 pt Gauss quadrature for variance calcs (Jul 2/03)
!---------------------------------------------------------------------------
...
!     data q/-.906179845938664d0,-.538469310105683d0,.000000000000000d0,
...
!     data q/-.989400934991649932596d0, -.944575023073232576078d0,
