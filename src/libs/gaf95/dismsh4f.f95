...
!					get range in greyscales (if lgrey)
      ...
      sdl=sqrt(alog(1+(c(2)/c(1))**2)*vf)
	  meanl=alog(c(1))-0.5*sdl**2
	  po3sd=exp(meanl+3.0*sdl)
	  ne3sd=exp(meanl-3.0*sdl)
      if( lgrey ) then
         if( llog ) then
			pmin = alog(prop(1))
            pmax = alog(prop(1))
                 ...
                 if( ldet ) then
                  ...
                  if( llog ) then
                   gr = scgr*(pmax - alog(prop(ii)))
                    ...
                    if(prop(ii)>po3sd)then
					gr=0.0
                    elseif(prop(ii)<ne3sd)then
					gr=1.0
					else
                 ...
                 if( ldetb ) then
                  ...
                  if( llog ) then
                   gr = scgrb*(pmaxb - alog(propb(ii)))
                    ...
                    if(propb(ii)>po3sd)then
					gr=0.0
                    elseif(propb(ii)<ne3sd)then
					gr=1.0
					else
